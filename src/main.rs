...
use ragit_api::{Model, ModelRaw, get_model_by_name, ApiConfig, Request, Message};
use ragit_api::qa_system::{ModelQASystem, ModelQAResult};
use ragit_cli::{
    ArgCount,
    ArgParser,
    ArgType,
...
mod log_qa_results;
use log_qa_results::log_qa_results;
...
async fn run(args: Vec<String>) -> Result<(), Error> {
    ...
    match args.get(1).map(|arg| arg.as_str()) {
        ...
        Some("qa-test") => {
            ...
            let qa_system = ModelQASystem::new(models, config);
            let request = Request {
                messages: vec![Message::simple_user_message(&prompt)],
                model: Model::dummy(), // Will be overridden per model
                ..Request::default()
            };
            let results = qa_system.test_request(request).await?;
            println!("{}", qa_system.compare_results(&results));
            log_qa_results(&results)?;
            Ok(())
        },
        Some("qa-tune") => {
            let parsed_args = ArgParser::new()
                .args(ArgType::Path, ArgCount::Exact(1))
                .args(ArgType::String, ArgCount::Exact(1))
                .args(ArgType::Float, ArgCount::Exact(1))
                .parse(&args, 2)?;

            if parsed_args.show_help() {
                // TODO: Add help message for qa-tune
            ...
            let user_score = parsed_args.get_args_exact(3)?[0].parse::<f64>().unwrap();
