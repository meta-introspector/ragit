...
impl Index {
    pub async fn add_image_description(&self, uid: Uid) -> Result<(), Error> {
        let description_path = Index::get_uid_path(
            path_utils::pathbuf_to_str(&self.root_dir),
            CHUNK_DIR_NAME,
            uid,
            Some("json"),
        )?;
        ...
        let image_bytes = read_bytes(path_utils::pathbuf_to_str(&image_path))?;
        let image_bytes = encode_base64(&image_bytes);

        if let Ok(j) = read_string(path_utils::pathbuf_to_str(&description_path)) {
