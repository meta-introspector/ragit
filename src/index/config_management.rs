...
impl Index {
    pub fn get_config_by_key(&self, key: String) -> Result<Value, Error> {
        let value = match key.as_str() {
            ...
            "max_chunk_size" => serde_json::to_value(&self.build_config.max_chunk_size)?,
    ...
    pub fn set_config_by_key(&mut self, key: String, value: String) -> Result<Option<Value>, Error> {
        let previous_value = match key.as_str() {
            ...
            "max_chunk_size" => {
                let prev = self.build_config.max_chunk_size;
                self.build_config.max_chunk_size = value.parse()?;
                serde_json::to_value(prev).ok()
            },
            "max_summary_len" => {
    ...
    pub fn get_all_configs(&self) -> Result<Vec<(String, Value)>, Error> {
        ...
        // BuildConfig
        configs.push(("max_chunk_size".to_string(), serde_json::to_value(&self.build_config.max_chunk_size)?));
