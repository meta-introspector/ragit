...
impl Index {
    ...
    pub async fn retrieve_chunks(
        ...
        let tfidf_limit = if super_rerank {
            max_summaries * 4
        } else {
            max_summaries
        };
        ...
        while chunks.len() > max_summaries {
            ...
            for cc in chunks.chunks(slide_size) {
                ...
                join_set.spawn(async move {
                    index
                        .summaries_to_chunks(&query, cc, max_retrieval.max(max_summaries / 2))
                        .await
                });
            }
        ...
        if chunks.len() > max_retrieval {
            chunks = self
                .summaries_to_chunks(query, chunks, max_retrieval)
                .await?;
        }
