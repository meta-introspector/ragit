...
pub fn gen_fsm(debug: bool) {
    let rules = vec![
        (
            TermKind::종성Term,
            vec![vec!["은", "이", "을", "과", "으로", "이랑", "이라고"]],
        ),
        (
            TermKind::Non종성Term,
            vec![vec!["는", "가", "를", "와", "랑", "라고"]],
        ),
        (
            TermKind::Any,
            vec![vec![
                "의", "만", "도", "에", "에서", "에게", "로", "까지", "부터", "한테", "하고", "께",
            ]],
        ),
        (
            TermKind::Any,
...
fn disassemble_자모s(
    rules: Vec<(TermKind, Vec<Vec<&str>>)>,
) -> Vec<(TermKind, Vec<Vec<Vec<자모>>>)> {
    rules
        .into_iter()
        .map(|(term, suffix)| {
            (
...
fn flatten_term_kinds(
    mut rules: Vec<(TermKind, Vec<Vec<Vec<자모>>>)>,
) -> Vec<(TermKind, Vec<Vec<Vec<자모>>>)> {
    let 중성_자모s = 중성S
        .iter()
        .map(|j| vec![자모::중성(*j)])
...
fn flatten_rules1(
    rules: Vec<(TermKind, Vec<Vec<Vec<자모>>>)>,
) -> Vec<(TermKind, Vec<Vec<자모>>)> {
    ...
    for (term, mut suffix) in rules.into_iter() {
        loop {
            if suffix.len() == 1 {
                ...
                for j1 in l1.iter() {
                    for j2 in l2.iter() {
                        permutation.push(vec![j1.clone(), j2.clone()].concat());
                    }
                }
...
fn generate_code(states: Vec<State>, debug: bool) -> String {
    ...
    lines.push((
        0,
        String::from("// This code is auto-generated by generator.rs"),
    ));
    ...
    if debug {
        ...
        lines.push((
            2,
            String::from(
                "println!(\"curr_char: {curr_char:?}, {:?}\", curr_char.map(|c| c.into_char()));",
            ),
        ));
    }
    ...
    lines.push((
        1,
        String::from("assemble(&자모s[..(자모s.len() - cut_at)].to_vec())"),
    ));
