...
pub async fn get_pool() -> &'static PgPool {
    POOL.get_or_init(|| async {
        write_log("init_pg_pool", "start initializing pg pool");
            ...
            .await
        {
            Ok(pool) => pool,
            Err(e) => {
                write_log("init_pg_pool", &format!("{e:?}"));
                panic!("{e:?}");
            }
        }
    })
    ...
    match r {
        ...
        Err((code, error)) => {
            write_log(
                ...
                &format!(
                    "code: {code}, error: {}",
                    trim_long_string(&error, 200, 200)
                ),
            );
...
impl<T> HandleError<T> for Option<T> {
    fn handle_error(self, code: u16) -> Result<T, (u16, String)> {
        self.ok_or_else(|| {
            (
                code,
                format!("expected type `{}`, got `None`", std::any::type_name::<T>()),
            )
        })
    }
}
...
impl HandleError<()> for bool {
    fn handle_error(self, code: u16) -> Result<(), (u16, String)> {
        if self {
            ...
            Err((code, String::from("expected `true`, got `false`")))
        }
    }
}
