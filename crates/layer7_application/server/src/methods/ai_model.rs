...
pub async fn get_ai_model_list(_query: HashMap<String, String>) -> Box<dyn Reply> {
...
//     let ai_models = ai_model::get_list(name, tags, limit, offset, pool)
//         .await
//         .handle_error(500)?;
//     Ok(Box::new(json(&ai_models)))
// }

pub async fn put_ai_model_list(_model: Value, _api_key: Option<String>) -> Box<dyn Reply> {
    panic!("FIX ME LATER: Fix the bootstrap first and this code later.");
}
