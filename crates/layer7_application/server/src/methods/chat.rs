...
//     repo::check_auth(repo_id, RepoOperation::Chat, api_key, pool)
//         .await
//         .handle_error(500)?
//         .handle_error(404)?;
//     let chat_id = chat_id.parse::<i32>().handle_error(400)?;
//     let chat = chat::get_chat_with_history_by_id(chat_id, pool)
//         .await
//         .handle_error(404)?;
...
//     let chats = chat::get_list_by_repo_id(repo_id, limit, offset, pool)
//         .await
//         .handle_error(500)?;
...
//         }
//     };
//     let index_at = join3(&config.repo_data_dir, &user, &repo).handle_error(400)?;
...
//         .handle_error(404)?;
//     let chat_id = chat_id.parse::<i32>().handle_error(400)?;
//     let chat = chat::get_chat_by_id(chat_id, pool)
//         .await
//         .handle_error(404)?;
...
//     let mut model_name = ai_model::get_default_model_name(&user, pool)
//         .await
//         .handle_error(500)?;
...
//     let model_schema = ai_model::get_model_schema(&user, &model_name, pool)
//         .await
//         .handle_error(400)?;
...
//     let history = chat::get_history_by_id(chat_id, pool)
//         .await
//         .handle_error(500)?;
...
pub async fn create_chat(_user: String, _repo: String, _api_key: Option<String>) -> Box<dyn Reply> {
    panic!("FIX ME LATER: Fix the bootstrap first and this code later.");
}
...
//         .handle_error(500)?
//         .handle_error(404)?;
//     let chat_id = chat::create_and_return_id(repo_id, pool)
//         .await
//         .handle_error(500)?;
