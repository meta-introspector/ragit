...
use ragit::Index;
use ragit_fs::{
    WriteMode, create_dir_all, join, join3, read_dir, remove_dir_all, write_bytes, write_log,
};
use warp::filters::multipart::FormData;
use warp::http::StatusCode;
use warp::reply::{Reply, with_header, with_status};

pub async fn post_begin_push(
    user: String,
    repo: String,
...
async fn post_begin_push_(user: String, repo: String, api_key: Option<String>) -> RawResponse {
    ...
    repo::check_auth(repo_id, RepoOperation::Push, api_key, pool)
        .await
        .handle_error(500)?
        .handle_error(404)?;
    let session_id = archive::create_new_session(repo_id, pool)
        .await
        .handle_error(500)?;

    create_dir_all(&join(&config.push_session_dir, &session_id).handle_error(500)?)
        .handle_error(500)?;
...
async fn post_archive_(_user: String, _repo: String, form: FormData) -> RawResponse {
    ...
    let session_id = form
        .get("session-id")
        .ok_or_else(|| "session-id not found")
        .handle_error(400)?;
    let session_id = String::from_utf8_lossy(session_id).to_string();
    let archive_id = form
        .get("archive-id")
        .ok_or_else(|| "archive-id not found")
        .handle_error(400)?;
    ...
    let archive = form
        ...
        .handle_error(400)?;
    archive::add_archive(&session_id, &archive_id, &archive, pool)
        .await
        .handle_error(500)?;

    let path = join3(&config.push_session_dir, &session_id, &archive_id).handle_error(400)?;

    write_bytes(&path, &archive, WriteMode::AlwaysCreate).handle_error(500)?;
...
async fn post_finalize_push_(user: String, repo: String, body: Bytes) -> RawResponse {
    ...
    let archives_at = join(&config.push_session_dir, &session_id).handle_error(400)?;
    let repo_id = repo::get_id(&user, &repo, pool).await.handle_error(404)?;
    let archives = read_dir(&archives_at, false).handle_error(404)?;
    let root_dir = join3(&config.repo_data_dir, &user, &repo).handle_error(400)?;
    ...
    archive::finalize_push(repo_id, &session_id, push_result, pool)
        .await
        .handle_error(500)?;
    remove_dir_all(&join(&config.push_session_dir, &session_id).handle_error(500)?)
        .handle_error(500)?;
