...
async fn get_user_list_(query: HashMap<String, String>, api_key: Option<String>) -> RawResponse {
    ...
    let users = user::get_list(include_privates, limit, offset, pool)
        .await
        .handle_error(500)?;
...
async fn get_user_ai_model_list_(user: String, api_key: Option<String>) -> RawResponse {
    ...
    let model_list = ai_model::get_list_by_user_id(&user, pool)
        .await
        .handle_error(500)?;
    Ok(Box::new(json(&model_list)))
}

pub async fn put_user_ai_model_list(
    user: String,
    form: Value,
    api_key: Option<String>,
) -> Box<dyn Reply> {
...
async fn put_user_ai_model_list_(
    user: String,
    form: Value,
    api_key: Option<String>,
) -> RawResponse {
    ...
    ai_model::register(&user, &update, pool)
        .await
        .handle_error(500)?;
