...
pub fn process_staged_file(
    ...
    memory_monitor.verbose(&format!("DEBUG: Processing file: {:?}", file_path_buf));
    let content = fs::read_to_string(file_path_buf)?;
    memory_monitor.verbose(&format!("DEBUG: Content of file {:?}: 
{}", file_path_buf, content));
    memory_monitor.verbose(&format!("DEBUG: Content length: {}", content.len()));
    memory_monitor.verbose(&format!("DEBUG: Chunk size: {}", build_config.chunk_size));
    let chunks: Vec<&str> = splitter.chunks(&content, build_config.chunk_size).collect();
    memory_monitor.verbose(&format!("DEBUG: Number of chunks generated by splitter for file {:?}: {}", file_path_buf, chunks.len()));
