...
pub async fn build_index(
    ...
    if verbose { println!("DEBUG: Chunks in index before processing files: {}", index.chunks.len()); }
    ...
    for file_path_buf in &staged_files_cloned {
        if verbose { println!("DEBUG: Processing file: {:?}", file_path_buf); }
        let content = fs::read_to_string(file_path_buf)?;
        if verbose { println!("DEBUG: Content of file {:?}: \n{}", file_path_buf, content); }
        if verbose { println!("DEBUG: Content length: {}", content.len()); }
        ...
        if verbose { println!("DEBUG: Number of chunks generated by splitter for file {:?}: {}", file_path_buf, chunks.len()); }
        ...
        for chunk_data in chunks {
            ...
            if verbose { println!("DEBUG: Adding chunk to index: data='{}', uid='{}'", new_chunk.data, new_chunk.uid); }
            index.add_chunk(new_chunk);
            chunk_index += 1;
        }
    }
    if verbose { println!("DEBUG: Total chunks added to index: {}", index.chunks.len()); }
    if verbose { println!("DEBUG: Chunks in index after processing files: {}", index.chunks.len()); }
    if verbose { println!("DEBUG: Chunks in index after processing files: {}", index.chunks.len()); }
