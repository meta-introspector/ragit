...
pub async fn process_single_chunk(
    ...
    if verbose { // Log every chunk
        ...
        ragit_utils::memory_utils::print_memory_usage(sys, &format!("Before loading chunk (chunk {}) (Call: {})", processed_chunks_count + 1, call_count), last_snapshot_data);
    }
    check_memory_limit(sys, max_memory_gb, &format!("Before loading chunk (chunk {}) (Call: {})", processed_chunks_count + 1, call_count))?;

    if verbose {
        ragit_utils::memory_utils::print_memory_usage(sys, &format!("After loading chunk (chunk {}) (Call: {})", processed_chunks_count + 1, call_count), last_snapshot_data);
    ...
    if verbose {
        print_memory_usage(sys, &format!("After appending chunk (chunk {}) (Call: {})", processed_chunks_count + 1, call_count), last_snapshot_data);
    }
