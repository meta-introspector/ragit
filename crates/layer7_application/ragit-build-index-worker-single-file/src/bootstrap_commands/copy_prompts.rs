...
pub async fn copy_prompts(
    ...
    memory_monitor.verbose("bootstrap_index_self: Copying prompts");
    ...
    for entry in fs::read_dir(prompts_dir)? {
        ...
        if path.is_file() {
            ...
            memory_monitor.verbose(&format!(r#"bootstrap_index_self: Copied prompt {:?} to {:?}"#, path, dest_path));
            if path.file_name().unwrap() == "summarize.pdl" {
                memory_monitor.verbose(&format!("DEBUG: summarize.pdl copied to: {:?}", dest_path));
            }
        }
