        ...
        MessageContent::Image { image_type, bytes } => match api_provider {
            ApiProvider::Anthropic => {
                ...
                source.insert(
                    ...
                    image_type.get_media_type().into(),
                );
            ...
            _ => {
                // assume the others are all openai-compatible
                let mut content = Map::new();
...
pub fn message_contents_to_json_array(
    contents: &[MessageContent],
    api_provider: &ApiProvider,
) -> Value {
    match api_provider {
        ApiProvider::Google => Value::Array(
            contents
                .iter()
                .map(|content| message_content_to_json(content, api_provider))
                .collect(),
        ),
        _ => {
...
pub fn message_to_json(message: &Message, api_provider: &ApiProvider) -> Value {
    ...
    result.insert(
        ...
        message
            .role
            .to_api_string(matches!(api_provider, ApiProvider::Google))
            .into(),
    );
    ...
    match (api_provider, message.content.len()) {
        ...
        (ApiProvider::Google, _) => {
            result.insert(
                String::from("parts"),
                message_contents_to_json_array(&message.content, api_provider),
            );
        }
        (ApiProvider::Anthropic, 1) if matches!(&message.content[0], MessageContent::String(_)) => {
