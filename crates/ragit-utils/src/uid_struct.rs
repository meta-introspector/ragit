...
use ragit_fs::{read_bytes, write_bytes, write_string, file_size, get_relative_path, WriteMode, read_bytes_offset, is_dir, extension, file_name, join, join3};
use crate::error::Error;
use crate::index::index_struct::Index;
use crate::constant::{CHUNK_DIR_NAME, FILE_INDEX_DIR_NAME, IMAGE_DIR_NAME};
...
pub fn save_to_file(
    ...
    match write_mode {
        UidWriteMode::Compact if uids.len() > 1 => {
            ...
            //    It intentionally avoids ascii code area area so that `load_from_file` can
...
impl Index {
    ...
    pub fn get_all_chunk_uids(&self) -> Result<Vec<Uid>, Error> {
        ...
        for internal in read_dir(path_utils::str_to_path_ref(&join3(path_utils::pathbuf_to_str(&self.root_dir), CHUNK_DIR_NAME, CHUNK_DIR_NAME)?), false).unwrap_or(vec![]) {
            let prefix = file_name(path_utils::str_to_path_ref(&path_utils::pathbuf_to_str(&internal)))?;

            if !is_dir(path_utils::str_to_path_ref(path_utils::pathbuf_to_str(&internal))) {
                continue;
            }

            for chunk_file in read_dir(path_utils::str_to_path_ref(path_utils::pathbuf_to_str(&internal)), false)? {
                if extension(path_utils::str_to_path_ref(&path_utils::pathbuf_to_str(&chunk_file)))?.unwrap_or(String::new()) != "chunk" {
                    continue;
                }

                result.push(Uid::from_prefix_and_suffix(&prefix, &file_name(path_utils::str_to_path_ref(&path_utils::pathbuf_to_str(&chunk_file)))?)?);
            }
        }
    ...
    pub fn get_all_image_uids(&self) -> Result<Vec<Uid>, Error> {
        ...
        for internal in read_dir(path_utils::str_to_path_ref(&join3(path_utils::pathbuf_to_str(&self.root_dir), IMAGE_DIR_NAME, IMAGE_DIR_NAME)?), false).unwrap_or(vec![]) {
            let prefix = file_name(path_utils::str_to_path_ref(&path_utils::pathbuf_to_str(&internal)))?;
            ...
            for image_file in read_dir(path_utils::str_to_path_ref(path_utils::pathbuf_to_str(&internal)), false)? {
                if extension(path_utils::str_to_path_ref(&path_utils::pathbuf_to_str(&image_file)))?.unwrap_or(String::new()) != "png" {
                    continue;
                }

                result.push(Uid::from_prefix_and_suffix(&prefix, &file_name(path_utils::str_to_path_ref(&path_utils::pathbuf_to_str(&image_file)))?)?);
            }
        }
