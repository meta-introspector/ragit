...
pub use ragit_core_utils::path_utils::{
    path_to_display, pathbuf_to_str, str_to_path_ref, str_to_pathbuf,
};
...
pub fn join_paths(path: &Path, child: &Path) -> Result<PathBuf, Error> {
    let joined = join(
        &pathbuf_to_str(&path.to_path_buf()),
        &pathbuf_to_str(&child.to_path_buf()),
    )?;
...
pub fn join3_paths(path1: &Path, path2: &Path, path3: &Path) -> Result<PathBuf, Error> {
    let joined = join3(
        &pathbuf_to_str(&path1.to_path_buf()),
        &pathbuf_to_str(&path2.to_path_buf()),
        &pathbuf_to_str(&path3.to_path_buf()),
    )?;
    Ok(PathBuf::from(joined))
}
...
pub fn get_rag_path(root_dir: &PathBuf, rel_path: &PathBuf) -> Result<PathBuf, Error> {
    ...
    Ok(str_to_pathbuf(
        normalize(&pathbuf_to_str(&joined_path))?.as_str(),
    ))
}
...
pub fn get_uid_path(
    ...
    let path = join3_paths(root_dir, &str_to_pathbuf(dir), &str_to_pathbuf(uid_prefix))?;

    let final_path = join_paths(path.as_path(), &str_to_pathbuf(uid_suffix))?;
...
pub fn get_ii_path_str(root_dir: &str, term_hash: String) -> PathBuf {
    let ii_at = join3(root_dir, INDEX_DIR_NAME, II_DIR_NAME).unwrap();
    let term_hash_prefix = term_hash.get(0..2).unwrap().to_string();
    let term_hash_suffix = term_hash.get(2..).unwrap().to_string();

    join3(&ii_at, &term_hash_prefix, &term_hash_suffix)
        .unwrap()
        .into()
}
